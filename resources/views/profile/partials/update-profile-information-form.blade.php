<section>
    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6">
        ...
        @php
            $nameParts = \Illuminate\Support\Str::of($user->name)->trim()->explode(' ')->filter()->values();
            $defaultFirstName = filled($user->first_name)
                ? $user->first_name
                : ($residentProfile?->first_name ?: (string) ($nameParts->first() ?? ''));
            $defaultMiddleName = filled($user->middle_name) ? $user->middle_name : ($residentProfile?->middle_name ?? '');
            $fallbackLast = $nameParts->count() > 1 ? (string) $nameParts->last() : (string) ($nameParts->first() ?? '');
            $defaultLastName = filled($user->last_name)
                ? $user->last_name
                : ($residentProfile?->last_name ?: $fallbackLast);
            $defaultSuffix = filled($user->suffix) ? $user->suffix : ($residentProfile?->suffix ?? '');
        @endphp
        ...
        <div class="grid gap-5 sm:grid-cols-2">
            ...
            <div>
                ...
                <x-text-input id="first_name" name="first_name" type="text" class="mt-1 block w-full" :value="old('first_name', $defaultFirstName)" required autofocus autocomplete="given-name" />
                <x-input-error class="mt-2" :messages="$errors->get('first_name')" />
            </div>
            <div>
                <x-input-label for="middle_name" :value="__('Middle Name')" />
                <x-text-input id="middle_name" name="middle_name" type="text" class="mt-1 block w-full" :value="old('middle_name', $defaultMiddleName)" />
                <x-input-error class="mt-2" :messages="$errors->get('middle_name')" />
            </div>
            <div>
                <x-input-label for="last_name" :value="__('Last Name')" />
                <x-text-input id="last_name" name="last_name" type="text" class="mt-1 block w-full" :value="old('last_name', $defaultLastName)" required autocomplete="family-name" />
                <x-input-error class="mt-2" :messages="$errors->get('last_name')" />
            </div>
            <div>
                <x-input-label for="suffix" :value="__('Suffix')" />
                <x-text-input id="suffix" name="suffix" type="text" class="mt-1 block w-full" :value="old('suffix', $defaultSuffix)" />
